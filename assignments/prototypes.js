...
Villian.prototype.stab = function (target) {
...
Hero.prototype.slash = function (target) {
  target.healthPoints = target.healthPoints - 1;
  if (target.healthPoints < 1) {
    console.log(target.destroy());
  } else {
    console.log(`${this.name} slashed ${target.name}! 1 Damage dealt. ${target.healthPoints} health points remaining!`);
  }
};
...
penguin.stab(vladimir);
vladimir.slash(penguin);
penguin.stab(vladimir);
vladimir.slash(penguin);
penguin.stab(vladimir);
vladimir.slash(penguin);
